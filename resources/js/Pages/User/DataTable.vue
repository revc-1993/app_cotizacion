<template>
    <Card class="bg-white">
        <template #body>
            <div
                class="flex w-full sm:w-auto items-center space-x-2 justify-end mb-3"
            >
                <Link
                    :href="route('superuser.user.create')"
                    as="button"
            ...
            <DataTable
                route="api.v1.superuser.user.paginate"
                :softDeletes="true"
                :interval="5000"
            >
                ...
                <template v-slot:body="props">
                    <tr class="hover:bg-slate-100">
                        <td
                            class="sticky top-0 left-0 border border-slate-200 text-center"
                        >
                            {{ props.i + 1 }}
                        </td>
                        <td class="border border-slate-200 p-2">
                            <img
                                ...
                                class="w-12 h-12 border border-slate-200 rounded-full mx-auto"
                            />
                        </td>
                        <td
                            class="border border-slate-200 px-3 py-1 capitalize"
                        >
                            {{ props.item.name }}
                        </td>
                        <td class="border border-slate-200 px-3 py-1 lowercase">
                            {{ props.item.username }}
                        </td>
                        ...
                        <td class="border border-slate-200 px-3 py-1">
                            {{
                                ...
                                new Date(props.item.deleted_at).toLocaleString(
                                    "id"
                                )
                            }}
                        </td>
                        <td class="border border-slate-200 px-3 py-1">
                            <BtnSplit
                                class="bg-slate-700 border-slate-800 text-slate-50 hover:bg-slate-800"
                                :r="248"
                                :g="250"
                                :b="252"
                            >
                                ...
                                <template #actions>
                                    <div
                                        class="absolute top-[102.5%] right-0 grid bg-slate-700 border border-slate-800 rounded shadow z-10"
                                    >
                                        <Link
                                            v-if="
                                            ...
                                            "
                                            class="text-sm uppercase px-3 py-1 hover:bg-slate-800"
                                        >
                                            {{ __("profile") }}
                                        </Link>
                                        ...
                                        <button
                                            v-if="
                                                props.item.deleted_at === null
                                            "
                                            @click.prevent="reset(props.item)"
                                            class="text-sm uppercase px-3 py-1 hover:bg-slate-800"
                                        >
                                            {{ __("reset password") }}
                                        </button>
...
export default defineComponent({
    ...
    methods: {
        destroy(user, refresh) {
            return Swal.fire({
                icon: "question",
                html: `<span class="first-letter:capitalize lowercase">${__(
                    user.deleted_at
                        ? "are you want to delete permanently"
                ...
                if (response.isConfirmed) {
                    Inertia.delete(
                        route("superuser.user.destroy", user.id) +
                            (user.deleted_at ? "?force=1" : "")
                    );
        ...
        reset(user) {
            return Inertia.patch(
                route("superuser.user.reset-password", user.id)
            );
        },
