<template>
    <div class="bg-inherit p-0 rounded-md">
        <div
            class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 sm:items-center justify-between mb-2"
        >
            <div
                class="flex w-full sm:w-auto items-center space-x-2 justify-between sm:justify-start"
            >
                ...
                <select
                    ...
                    class="bg-inherit border border-slate-300 rounded-md p-1"
                >
                ...
                <button
                    v-if="softDeletes"
                    @click.prevent="
                        config.withTrashed = !config.withTrashed;
                        refresh();
                    "
                    class="border border-slate-300 rounded px-3 py-1 uppercase"
                >
                    {{
                        __(
                            config.withTrashed
            ...
            <input
                ...
                class="bg-inherit sm:w-2/5 border border-slate-300 rounded-md placeholder:capitalize text-xs"
        ...
        <transition name="fade">
            <div
                class="overflow-auto border border-slate-300 rounded-md mb-2 max-h-[25rem]"
            >
        ...
        <div class="flex items-center">
            <div class="flex-none w-1/4">
                <p class="lowercase first-letter:capitalize text-xs">
                    {{ __("displaying") }} {{ data.from }} {{ __("to") }}
                    {{ data.to }} {{ __("from") }} {{ data.total }}
                </p>
            </div>
            ...
            <div class="flex flex-wrap w-full items-center justify-end">
                <div
                    ...
                    class="text-xs border p-1 shadow cursor-pointer"
                ></div>
            </div>
        </div>
    </div>
...
export default defineComponent({
    ...
    methods: {
        refresh() {
            ...
            if (this.config.sort.key.trim() && this.config.sort.order.trim())
                data.sort = {
                    key: this.config.sort.key,
                    order: this.config.sort.order,
                };
            if (this.softDeletes)
                data.withTrashed = this.config.withTrashed ? 1 : 0;
